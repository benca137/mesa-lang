...
class MIRGenerator:
    ...
    def _lower_expr(self, expr: Optional[Expr], preferred_name: Optional[str] = None) -> str:
        ...
        if isinstance(expr, BlockExpr):
            ...
            if loop_stmt is not None:
                return self._lower_loop_expr(loop_stmt, expr, preferred_name=preferred_name)
    ...
    def _block_loop_expr_stmt(self, block: Block):
    ...
    def _lower_loop_expr(self, stmt, expr: BlockExpr, *, preferred_name: Optional[str]) -> str:
        if not isinstance(stmt, WhileStmt):
            raise MIRGenError("only while loop expressions are lowered right now")
        value = self._lower_while(stmt, result_type=self._expr_type_text(expr))
        if value is None:
            raise MIRGenError("while expression did not produce a loop result")
        if preferred_name is None:
            return value
        return self.emit_assign(
            self._fresh_value_name(preferred_name),
    ...
    def _prepare_optional_loop_result_binding(self, result_type: str) -> str:
        result_name = self._loop_result_name()
        result_value = self.emit_assign(
            self._fresh_value_name("loop_value"),
            "none",
    ...
    def _lower_while(self, stmt: WhileStmt, *, result_type: Optional[str] = None) -> Optional[str]:
        result_name = (
            self._prepare_optional_loop_result_binding(result_type)
            if result_type is not None
            else self._prepare_loop_result_binding(stmt.body, stmt.label)
        )
        before = self.snapshot_scopes()
        ...
        self._loop_stack.append(
            LoopContext(
                ...
                result_type=result_type,
                result_kind="optional" if result_type is not None else ("raw" if result_name is not None else None),
            )
        )
        try:
        ...
        return self.lookup_binding(result_name).ssa_name if result_name is not None else None
        ...
        try:
            ...
            self._loop_stack.append(
                LoopContext(
                    ...
                    result_name=result_name,
                    result_type=None,
                    result_kind="raw" if result_name is not None else None,
                )
            )
            try:
                self.switch_to(cond_block)
    ...
    def _lower_break(self, stmt: BreakStmt):
        ...
        if break_value is not None and loop.result_name is not None:
            ...
            if loop.result_kind == "optional":
                if loop.result_type is None:
                    raise MIRGenError("optional loop result is missing its type")
                wrapped_value = self.emit_assign(
                    self._fresh_value_name("loop_result"),
                    f"optional.some({break_value})",
                    loop.result_type,
                    annotate_type=True,
                )
                break_args[result_index] = wrapped_value
            else:
